...
serv_out[["coilrgl"]] <- function(input,session){
  ...
  mqd <- reactive({
    ...
    qd <- qd[,c(2,3,1)]
    qd[,2] <- -1*qd[,2]
    qd <- apply(qd,2,function(x)x/max(abs(range(qd$X,qd$Y,qd$Z)))) #Rescale
    if(W>50){ #For large W delete turns that are too small to be seen
      qd <- qd[apply(qd,1,function(x)any(abs(x)>1e-5)),]
      if(nrow(qd)%%4) qd <- rbind(matrix(0,nrow=4-nrow(qd)%%4,ncol=3),qd)
    }
    as.mesh3d(qd,type="quads")
    })
  # Orientation
...
serv_out[["paramstab"]] <- function(input,session){
  renderText(sprintf("W=%s\tD=%s\tS=%s\tT=%s", round(10^input$W,2),input$D,input$S,input$RT))
}
